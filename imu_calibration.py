...
TICKS_MAX = 0xFFFFFFFF

def ticks_diff(end, start):
    return (end - start + (TICKS_MAX + 1)) % (TICKS_MAX + 1)
...
print("BNO08x Accelerometer report enabled\n")
...
while True:
    sleep(0.2)
    acc_x, acc_y, acc_z = bno.acc
    print(f"\nAccelerometer:  X: {acc_x:.4f}  Y: {acc_y:.4f}  Z: {acc_z:.4f} uT")

    # print accuracy value and string
    calibration_status = bno.calibration_status
    print(f"Acc Calibration: \"{REPORT_ACCURACY_STATUS[calibration_status]}\" = {calibration_status}")
    now = ticks_ms()
